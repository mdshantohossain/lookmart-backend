...
<div class="container">
            ...
            @if(session('payment_method') == 0)
                ...
                <li class="list-group-item d-flex justify-content-between lh-condensed">
                    <div class="justify-center">
                        <h6 class=" text-center">Pay your delivery charge <strong>{{ 'TK.'. session('shipping_cost') }}</strong> is  required.</h6>
                    </div>
                </li>
            ...
            @else
                ...
                <ul class="list-group mb-3">
                    @foreach(Cart::content() as $cartItem)
                        <li class="list-group-item d-flex justify-content-between lh-condensed">
                            <div>
                                <h6 class="my-0">{{ substr($cartItem->name, 0, 32) }}</h6>
                                <small class="text-muted">{{ $cartItem->qty }}</small>
                            </div>
                            <strong>{{ 'TK'. $cartItem->price }}</strong>
                        </li>
                    @endforeach
                    <li class="list-group-item d-flex justify-content-between">
                        @if(session('shipping_cost') != 0)
                            <span>Delivery Charge</span>
                            <strong>TK.{{ session('shipping_cost') }}</strong>
                        @else
                            <span>Delivery charge is free.</span>
                        @endif
                    </li>
                    <li class="list-group-item d-flex justify-content-between">
                        <span>Total (BDT)</span>
                        <strong>TK.{{ (float) str_replace(',', '', Cart::subTotal()) + session('shipping_cost') }} </strong>
                    </li>
    ...
    <footer class="my-5 pt-5 text-muted text-center text-small">
        <p class="mb-1">&copy
            <script>document.write(new Date().getFullYear())</script> Company Name</p>
